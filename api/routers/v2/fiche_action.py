...
router = APIRouter(prefix='/v2/fiche_action')


@router.post("/{epci_id}", response_model=FicheAction_Pydantic)
async def write_epci_fiche_action(
        epci_id: str,
        fiche_action: FicheActionIn_Pydantic,
    ...
    query = FicheAction.filter(
        epci_id=epci_id,
        uid=fiche_action.uid,
    )

    if await query.exists():
        await query.update(latest=False)

    fiche_action_obj = await FicheAction.create(
        **fiche_action.dict(exclude_unset=True),
        latest=True,
        deleted=False,
    )
...
async def get_all_epci_actions_status(epci_id: str):
    query = FicheAction.filter(epci_id=epci_id, latest=True, deleted=False)
...
async def get_fiche_action(epci_id: str, uid: str):
    query = FicheAction.get(epci_id=epci_id, uid=uid, latest=True, deleted=False)
    try:
